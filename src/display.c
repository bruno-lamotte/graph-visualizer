...
/*   Updated: 2026/02/06 01:10:31 by blamotte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_player(t_vars *vars, int steps, int nb_moves)
{
	...
	position = vars->current_state->y * vars->map.width + vars->current_state->x;
	...
	while (nb_moves > 0)
	{
		...
		if (current_time - last_update_time > 100)
		{
			mlx_put_image_to_window(vars->mlx, vars->win,
				vars->textures.floor,
				position % vars->map.width * TILE_SIZE,
				position / vars->map.width * TILE_SIZE);
			position += steps;
			mlx_put_image_to_window(vars->mlx, vars->win,
				vars->textures.player,
				position % vars->map.width * TILE_SIZE,
				position / vars->map.width * TILE_SIZE);
...
void render_game(t_vars *vars, int steps, int nb_moves)
{
    ...
    while (y < vars->map.height)
    {
        ...
        while (x < vars->map.width)
        {
            ...
            char c = vars->map.map[index];
            if (c == WALL_CHAR)
                img = vars->textures.wall;
            else if (c == HOLE_CHAR || (is_collected(vars, index) && c == BREAKABLE_CHAR))
...
int	handle_keypress(int keysym, t_vars *vars)
{
    ...
    if (move_dir != 0)
    {
        ...
        if (next_state != NULL)
        {
			...
			if (steps != 0)
			{
				int pos_current = vars->current_state->y * vars->map.width + vars->current_state->x;
				int pos_next = next_state->y * vars->map.width + next_state->x;
...
void	initialize_textures(t_vars *vars)
{
	...
	mlx = vars->mlx;
	vars->textures.wall = mlx_xpm_file_to_image(mlx, "./textures/wall.xpm", &w, &h);
	vars->textures.floor = mlx_xpm_file_to_image(mlx, "./textures/floor.xpm", &w, &h);
	vars->textures.player = mlx_xpm_file_to_image(mlx, "./textures/player.xpm", &w, &h);
	vars->textures.collectible = mlx_xpm_file_to_image(mlx, "./textures/coin.xpm", &w, &h);
	vars->textures.exit = mlx_xpm_file_to_image(mlx, "./textures/exit.xpm", &w, &h);
	vars->textures.fragile_floor = mlx_xpm_file_to_image(mlx, "./textures/breackable.xpm", &w, &h);
	vars->textures.hole = mlx_xpm_file_to_image(mlx, "./textures/hole.xpm", &w, &h);
}
